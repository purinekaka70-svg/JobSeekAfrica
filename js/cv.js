...
function getFormData() {
  ...
  return {
    template: form.cvTemplate.value,
    pages: form.cvPages?.value || "1",
...
function applyFormData(data) {
  ...
  form.cvTemplate.value = data.template || "modern";
  if (form.cvPages) {
    form.cvPages.value = data.pages || "1";
  }
...
function updatePreview(data) {
  ...
  previewEl.classList.add(`cv-${template}`);
